...
##### single task representation learning start #####
class TrainDataset(Dataset):
    '''
    This implementation is for multi-task training. All the datasets will be concatenated together.
    '''
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, data_args: DataArguments, trainer: DenseTrainer = None, shuffle_seed: int = None, cache_dir: str = None) -> None:
        ...
        self.data_files = data_args.train_path if data_args.train_dir is None else glob.glob(os.path.join(data_args.train_dir, "train.*.jsonl"))
        datasets = [load_dataset("json", data_files=f, streaming=False, cache_dir=cache_dir)["train"] for f in self.data_files]
    ...
    def save_task_n2id(self, output_dir):
        pickle.dump(self.task_n2id, open(os.path.join(output_dir, 'task_n2id.pkl'), 'wb'))

    def load_task_n2id(self, input_dir):
        self.task_n2id = pickle.load(open(os.path.join(input_dir, 'task_n2id.pkl'), 'rb'))
    ...
    def process_fn(self, example):
        ...
        # if example['task'] not in self.task_n2id:
        #     self.task_n2id[example['task']] = len(self.task_n2id)
        task_id = self.task_n2id[example['task']]

        encoded_query = self.create_one_example(example['q_text'])
        encoded_key = self.create_one_example(example['k_text'])

        return {"query": encoded_query, "key": encoded_key, "task_id": task_id}
...
class EvalDataset(TrainDataset):
    ...
    def __init__(self, tokenizer: PreTrainedTokenizer, mode: str, data_args: DataArguments, shuffle_seed: int = None, cache_dir: str = None) -> None:
        super(EvalDataset, self).__init__(tokenizer, data_args, None, cache_dir=cache_dir)

        # self.data_files = data_args.eval_path if data_args.train_dir is None else glob.glob(os.path.join(data_args.train_dir, "val.*.jsonl"))
        self.data_files = data_args.eval_path if data_args.train_dir is None else glob.glob(os.path.join(data_args.train_dir, f"{mode}.*.jsonl"))
    ...
    def __getitem__(self, index):
        ...
        return self.process_fn(self.dataset[index])
##### single task representation learning end #####


##### multi-task representation learning start #####
class TrainSingleDataset(Dataset):
    '''
    This implementation is for multi-task training. All the datasets will be concatenated together.
    '''

    def __init__(self, data_path: str, tokenizer: PreTrainedTokenizer, data_args: DataArguments, trainer: DenseTrainer = None, shuffle_seed: int = None, cache_dir: str = None) -> None:
        super(TrainSingleDataset, self).__init__()

        # self.data_files = data_args.train_path if data_args.train_dir is None else glob.glob(os.path.join(data_args.train_dir, "train.*.jsonl"))
        # datasets = [load_dataset("json", data_files=f, streaming=False, cache_dir=cache_dir)["train"] for f in self.data_files]
        # self.dataset = concatenate_datasets(datasets)
        self.dataset = load_dataset("json", data_files=data_path, streaming=False, cache_dir=cache_dir)["train"]
        self.dataset = self.dataset.shuffle(seed=shuffle_seed) if shuffle_seed is not None else self.dataset
...
class EvalSingleDataset(TrainSingleDataset):

    def __init__(self, data_path: str, tokenizer: PreTrainedTokenizer, mode: str, data_args: DataArguments, shuffle_seed: int = None, cache_dir: str = None) -> None:
        super(EvalSingleDataset, self).__init__(data_path, tokenizer, data_args, None, cache_dir=cache_dir)
    ...
    def __getitem__(self, index):
        ...
        return self.process_fn(self.dataset[index])
##### multi-task representation learning end #####
...
##### downstream matching task start #####
class TrainMatchDataset(Dataset):
    '''
    This implementation is for multi-task training. All the datasets will be concatenated together.
    '''

    def __init__(self, tokenizer: PreTrainedTokenizer, data_args: DataArguments, trainer: DenseTrainer = None, shuffle_seed: int = None, cache_dir: str = None) -> None:
        super(TrainMatchDataset, self).__init__()
...
class EvalMatchDataset(TrainMatchDataset):

    def __init__(self, tokenizer: PreTrainedTokenizer, mode: str, data_args: DataArguments, shuffle_seed: int = None, cache_dir: str = None) -> None:
        super(EvalMatchDataset, self).__init__(tokenizer, data_args, None, cache_dir=cache_dir)
    ...
    def __getitem__(self, index):
        ...
        return self.process_fn(self.dataset[index])
##### downstream matching task end #####


##### downstream classification task start #####
...
class EvalNCCDataset(TrainNCCDataset):
    ...
    def __getitem__(self, index):
        ...
        return self.process_fn(self.dataset[index])
##### downstream classification task end #####


##### downstream regressions task start #####
class TrainREGDataset(Dataset):

    def __init__(self, tokenizer: PreTrainedTokenizer, data_args: DataArguments, trainer: DenseTrainer = None, shuffle_seed: int = None, cache_dir: str = None) -> None:
        super(TrainREGDataset, self).__init__()
        self.data_files = data_args.train_path if data_args.train_dir is None else glob.glob(os.path.join(data_args.train_dir, "*.jsonl"))
        self.dataset = load_dataset("json", data_files=self.data_files, streaming=False, cache_dir=cache_dir)["train"]
        self.dataset = self.dataset.shuffle(seed=shuffle_seed) if shuffle_seed is not None else self.dataset
        self.tokenizer = tokenizer
    ...
    def process_fn(self, example):
        ...
        return {"query": encoded_query, 'label': float(example['label'])}

    def __getitem__(self, index):
        return self.process_fn(self.dataset[index])


class EvalREGDataset(TrainREGDataset):

    def __init__(self, tokenizer: PreTrainedTokenizer, data_args: DataArguments, shuffle_seed: int = None, cache_dir: str = None) -> None:
        super(EvalREGDataset, self).__init__(tokenizer, data_args, None, cache_dir=cache_dir)
    ...
    def __getitem__(self, index):
        ...
        return self.process_fn(self.dataset[index])
##### downstream regressions task end #####


##### downstream retrieval task start #####
class TrainHnDataset(Dataset):
    ...
    def process_fn(self, example, epoch, hashed_seed):
        ...
        return {"query": encoded_query, "key": encoded_keys}
