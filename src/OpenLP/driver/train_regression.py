...
from OpenLP.dataset import TrainREGCollator, TrainREGDataset, EvalREGDataset
from OpenLP.modeling import DenseModelforREG
from OpenLP.trainer import DenseTrainer as Trainer
from OpenLP.utils import calculate_reg_metrics
...
def main():
    ...
    #         model.lm.final_projection_layer.requires_grad = True
    ...
    train_dataset = TrainREGDataset(tokenizer, data_args, shuffle_seed=training_args.seed, cache_dir=data_args.data_cache_dir or model_args.cache_dir)
    eval_dataset = EvalREGDataset(tokenizer, data_args, shuffle_seed=training_args.seed, cache_dir=data_args.data_cache_dir or model_args.cache_dir) if data_args.eval_path is not None else None
    ...
    trainer = trainer_cls(
        ...
        data_collator=TrainREGCollator(
            tokenizer,
            max_len=data_args.max_len,
        ),
        callbacks=[tb_callback],
        compute_metrics=calculate_reg_metrics,
    )
