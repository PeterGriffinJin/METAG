# This code script is for contrastive training with given negative samples in addition to in batch negative samples.
...
from OpenLP.dataset import TrainHnCollator, TrainHnDataset, EvalHnDataset
...
def main():
    ...
    train_dataset = TrainHnDataset(tokenizer, data_args, shuffle_seed=training_args.seed, cache_dir=data_args.data_cache_dir or model_args.cache_dir)
    eval_dataset = EvalHnDataset(tokenizer, 'val', data_args, shuffle_seed=training_args.seed, cache_dir=data_args.data_cache_dir or model_args.cache_dir)
