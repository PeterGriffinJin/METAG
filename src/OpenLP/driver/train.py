...
def main():
    ...
    assert model_args.model_type in ['bert', 't5', 'bert_peft']
    ...
    eval_dataset = EvalDataset(tokenizer, 'val', data_args, shuffle_seed=training_args.seed, cache_dir=data_args.data_cache_dir or model_args.cache_dir)
    
    train_dataset.load_task_n2id(data_args.train_dir)
    eval_dataset.load_task_n2id(data_args.train_dir)

    assert len(train_dataset.task_n2id) == model_args.num_tasks
    assert len(eval_dataset.task_n2id) == model_args.num_tasks
