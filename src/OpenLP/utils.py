...
from sklearn.metrics import f1_score, roc_auc_score, recall_score, precision_score, accuracy_score, mean_squared_error, mean_absolute_error
...
class calculate_multitask_metrics:
    def __init__(self, train_dir):
        task_n2id = pickle.load(open(os.path.join(train_dir, 'task_n2id.pkl'), 'rb'))
        self.task_id2n = {task_n2id[n]:n for n in task_n2id}
    ...
    def __call__(self, evalpred: EvalPrediction):
        ...
        # obtain score and label for each task
        scores = []
        labels = []
        for i in range(len(evalpred.predictions[0])):
            scores.append(evalpred.predictions[0][i])
            labels.append(evalpred.predictions[1][i])
        ...
        for score, label in zip(scores, labels):
            ...
            prc = (np.sum((prediction == label)) / label.shape[0])
            prcs.append(prc)

            n_labels = np.max(label) + (label[1] - label[0])
            labelst = np.eye(n_labels)[label]

            mrr_all = [mrr_score(labelst[i], score[i]) for i in range(labelst.shape[0])]
            mrr = np.mean(mrr_all)
            mrrs.append(mrr)

            ndcg_10_all = [ndcg_score(labelst[i], score[i], 10) for i in range(labelst.shape[0])]
            ndcg_10 = np.mean(ndcg_10_all)
            ndcg10s.append(ndcg_10)

        # prepare result
        rmetrics = [{f'{self.task_id2n[i]}_prc': p, f'{self.task_id2n[i]}_mrr': m, f'{self.task_id2n[i]}_ndcg10': nd} for i, (p, m, nd) in enumerate(zip(prcs, mrrs, ndcg10s))]
        result_dict = dict(ChainMap(*rmetrics))
        result_dict['prc'] = np.mean(prcs)
        result_dict['mrr'] = np.mean(mrrs)
        result_dict['ndcg10'] = np.mean(ndcg10s)
...
def calculate_reg_metrics(evalpred: EvalPrediction):
    '''
    This function is for regression evaluation.
    '''
