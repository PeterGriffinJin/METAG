...
with open(os.path.join(f'xxx/data/{args.domain}/downstream/retrieval', 'documents.txt')) as f:
    readin = f.readlines()
...
qid = 0
with open(f'xxx/data/{args.domain}/downstream/retrieval/node_classification.jsonl') as f:
    readin = f.readlines()
    for line in tqdm(readin):
        tmp = json.loads(line)
        
        query_dict[qid] = {'q_text': tmp['q_text']}
        data[qid] = {'q_text': tmp['q_text'], 'positive_ctxs':tmp['labels'], 'negative_ctxs':[]}
        qid += 1
...
# add bm25 negative
with open(f'xxx/data/{args.domain}/downstream/retrieval/bm25_all_trec') as f:
    readin = f.readlines()
    for line in tqdm(readin):
...
# save
with open(f'xxx/data/{args.domain}/downstream/retrieval/node_retrieval.jsonl', 'w') as fout:
    random.shuffle(data)
    for d in data:
...
with open(f'xxx/data/{args.domain}/downstream/retrieval/node_retrieval.jsonl') as f, \
        open(f'xxx/data/{args.domain}/downstream/retrieval/train.text.jsonl', 'w') as fout1, \
            open(f'xxx/data/{args.domain}/downstream/retrieval/val.text.jsonl', 'w') as fout2, \
                open(f'xxx/data/{args.domain}/downstream/retrieval/test.truth.trec', 'w') as fout3, \
                    open(f'xxx/data/{args.domain}/downstream/retrieval/test.node.text.jsonl', 'w') as fout4, \
                        open(f'xxx/data/{args.domain}/downstream/retrieval/test.node.text.tsv', 'w') as fout5:
    readin = f.readlines()
    ...
    for line in tqdm(readin[:int(0.8*total_len)]):
        ...
        fout1.write(json.dumps({
            'q_text':tmp['q_text'],
            'positives':[{'k_text': lname} for lname in tmp['positive_ctxs']],
            'negatives':[{'k_text': lname} for lname in tmp['negative_ctxs']]
        })+'\n')
        docid += 1
